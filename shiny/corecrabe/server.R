    ...
    if("Tou-te-s" %in% input$village_valeur){
      if("Tou-te-s" %in% input$sous_zone_valeur){
        p <- ggplot(prix_moyenne, aes(x = Mois, y =prix_collecte1, group=Zone, color=Zone)) +
          ...
          scale_x_discrete(name ="",limits=filtre_mois[match(format(as.Date("2020/12/01",  format="%Y/%m/%d"),"%h %y"),filtre_mois):match(format(as.Date("2023/03/01",  format="%Y/%m/%d"),"%h %y"),filtre_mois)])+
    ...
    if("Tou-te-s" %in% input$village_valeur){
      if("Tou-te-s" %in% input$sous_zone_valeur){
        p <- ggplot(func_valeur(input$zone_valeur,input$village_valeur,input$date_valeur,input$engin_valeur,input$sous_zone_valeur), aes(x = Mois, y =valeur/1000000, group=Zone, fill=Zone,color=Zone)) +
          geom_line(aes(color=Zone))+
          scale_x_discrete(name ="",limits=filtre_mois[match(format(as.Date("2020/12/01",  format="%Y/%m/%d"),"%h %y"),filtre_mois):match(format(as.Date("2022/12/01",  format="%Y/%m/%d"),"%h %y"),filtre_mois)], breaks = c("Dec 20","Oct 21","Dec 21","Oct 22","Dec 22","Oct 23"))+
...
###################################################################################################  
  ...
  output$map <- renderLeaflet({
    ...
    leaflet() %>%
      ...
      addProviderTiles(providers$OpenStreetMap.Mapnik, group = "Carte du monde")%>%
      ...
      addCircles(data= filter(production_geo,!is.na(lng)),opacity = 1, radius = ~cumul/13, color = 'red',  group = 'Production totale par village', popup = ~as.character(
        paste(sep = "<br/>",
              paste(sep = " ","VILLAGE :",village),
              paste(sep = " ","PRODUCTION ANNUELLE :",round(cumul/1000),"t"),
              paste(sep = " ","SUIVI :",suivi)
              )
        ),
